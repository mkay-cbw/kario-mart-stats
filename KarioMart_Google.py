...
def speichere_in_cloud(force=False):
    """Schiebt die lokale Datenbank zu Google Sheets hoch, mit Cooldown-Schutz."""
    now = time.time()
    if not force and (now - st.session_state.last_sync < 15):
        return
    ...
    with st.spinner("💾 Speichere Daten in der Cloud..."):
        for tabelle in ALL_TABLES:
            try:
                ...
                st.error(f"Fehler bei Tabelle {tabelle}: {e}")
...
# Hilfsfunktion für Segmented Controls
def ui_platzierung_auswahl(name, prefix_key, default_val=None):
...
with tab1:
    if not st.session_state.authenticated:
        ...
        if not st.session_state.turnier_aktiv and not st.session_state.warten_auf_endplatzierung:
            ...
            for name in aktive_namen:
                val = ui_platzierung_auswahl(name, prefix_key="ep")
                if val in ["doppelt", "fehlt"]:
                    eingabe_fehler = True
                else:
...
# ==========================================
with tab2:
    st.write("#### Spieler-Leistungsprofile")
...
# ==========================================
with tab3:
    st.write("#### Strecken-Spezifische Statistiken")
...
with tab5:
    ...
    if df_verlauf.empty:
        ...
        if not st.session_state.authenticated:
            st.warning("🔒 Melde dich an, um den Verlauf zu editieren.")
        else:
            st.write("**Turnier-ID:**")
            ...
            if ausgewaehltes_turnier:
                ...
                if st.button("Aktualisieren", type="primary"):
                    if eingabe_fehler_ep:
                         st.error("❌ Ungültige Platzierungen gewählt!")
                    else:
                        c = conn.cursor()
                ...
                for idx, r_row in df_rennen_liste.iterrows():
                    ...
                    with st.expander(f"🏎️ Rennen {idx + 1} (ID #{r_id}): {r_row['strecken_name']}"):
                        ...
                        if st.button("Aktualisieren", key=f"btn_save_race_{r_id}"):
                            if eingabe_fehler_r:
                                st.error("❌ Ungültige Platzierungen gewählt!")
